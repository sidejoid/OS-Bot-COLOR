...
class OSRSWoodcutter(OSRSBot):
    ...
    def create_options(self):
        ...
        self.options_builder.add_checkbox_option("to_bank", "Bank logs?", [" "])
    ...
    def main_loop(self):
        ...
        while time.time() - start_time < end_time:
            ...
            if rd.random_chance(probability=0.02):
                if self.to_bank is False:
                    self.__drop_logs(api_m)
            ...
            probability = 0.10
            while api_m.get_is_player_idle() is False:
    ...
    def __drop_logs(self, api_m: MorgHTTPSocket):
        """
    ...
    def __bank_logs(self, api_m: MorgHTTPSocket):
        """
        banks logs
        """
        slots = api_m.get_inv_item_indices(ids.logs)
        ...
        banks = self.get_all_tagged_in_rect(self.win.game_view, clr.GREEN)
        bank = None
        if not banks:
            return False
        ...
        time.sleep(1)
        while True:
            if api_m.get_is_player_idle(1) is False:
                continue
            else:
                pag.press('4')
                break
